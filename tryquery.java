...
//		ArrayList<String> stuff = getScores("Hard");
//		Iterator<String> stuffIt = stuff.iterator();
//		
//		while (stuffIt.hasNext()) {
//			System.out.println(stuffIt.next());
//		}
		
		ArrayList<String> stuff = new ArrayList<String>();
		stuff.add("xXx420BlAzEiTxXx");
		stuff.add("0");
		stuff.add("HARD");
	...
	public static void storeEntry(ArrayList<String> theList ) {
		
		try{
			Connection conn = sqltest.getConnection();
			PreparedStatement ps = null;
			ps = conn.prepareStatement("INSERT INTO Leaderboard (Username , Score , Difficulty) VALUES (? , ? , ?)");
			ps.setString(1, theList.get(0));
			ps.setString(2, theList.get(1));
			ps.setString(3, theList.get(2));
			ps.executeUpdate();
